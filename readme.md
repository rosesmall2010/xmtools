...
- **作用**：递归把源目录下所有文件移动到目标目录，不保留相对路径（拍平）。是移动而非复制，源文件会被删除；移动后源目录下可能残留空子目录，可配合 `rmemptydir` 清理。
- **启动方式**：`node mvflat.js <源目录> <目标目录>`
- **启动参数**：
...
- **示例**：`node mvflat.js /data/music /data/music-flat`
